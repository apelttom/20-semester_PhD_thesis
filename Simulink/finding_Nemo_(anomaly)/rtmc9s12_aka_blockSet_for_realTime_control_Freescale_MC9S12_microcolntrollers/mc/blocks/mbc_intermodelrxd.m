function [buf_sizestr, channelstr, data_type_str, sampletime_str] = mbc_intermodelrxd(sampletime, channel, buf_size, data_type)
...
if sampletime < 0 && sampletime ~= -1
   fprintf ('InterModel_RX: Inadmissible sample time !\n');
...
if channel < 0 || channel > MAX_UCOM_CHANNELS
   channelstr = '???';
   fprintf ('InterModel_RX: Inadmissible channel number !\n');
else
...
if buf_size < 1 || buf_size > 100
   fprintf ('InterModel_RX: Inadmissible buffer size!\n');
   buf_sizestr = '???';
else
...
myReceiveBlocks = get_param(find_system(gcs, 'LookUnderMasks', 'all', 'Name', 'RECEIVE_FROM_MODEL'), 'Parent');     % get all RxD blocks
...
if(length(unique(kk)) < length(kk))
    ...
    disp('InterModel_RX: Warning... at least 2 RxD blocks have been allocated the same channel number!')
end
