% block mask: freeportRX
function [buf_sizestr, portNamestr, baudratestr, channelstr, data_type_str, sampletime_str] = ...
    mbc_freeportrxd(sampletime, port, baudrate, channel, buf_size, data_type, format, outformat)
...
if (sampletime < 0) && (sampletime ~= -1)
    ...
    % invalid sample time
    fprintf ('FreePortCommsRX: Inadmissible sample time !\n');
...
if (buf_size < 1) || (buf_size > 100)
    ...
    fprintf ('FreePortCommsRX: Inadmissible buffer size!\n');
    buf_sizestr = '???';
...
if format == 1
    ...
    % raw data
    set_param(gcb, 'MaskVisibilities', {'on', 'on', 'on', 'off', 'on', 'off', 'on', 'on' });
    % set_param(gcb, 'MaskEnables', {'on', 'on', 'on', 'off', 'on', 'on', 'on', 'on' });
...
if outformat == 1
    ...
    % force block output data type to 'double'
    set_param(myBlocks{DTC}, 'DataType', 'double');
    
else
    
    % no block output data type override
...
myReceiveBlocks = find_system(bdroot, 'BlockType', 'SubSystem', 'Tag', 'mcTarget_freePortCommsRX');
kk = str2double(get_param(myReceiveBlocks, 'channel'));
if length(unique(kk)) < length(kk)
    
    % the same channel number has been used more than once...
    disp('FreePortCommsRX: Warning... at least 2 RxD blocks have been allocated the same channel number!')
...
MC9S12DriverDataBlock = find_system(gcb, 'FollowLinks', 'on', 'LookUnderMasks', 'all', 'BlockType', 'S-Function', 'Tag', 'mcTarget_freePortCommsRX');
