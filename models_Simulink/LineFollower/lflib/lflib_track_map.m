function lflib_track_map(block)
setup(block);
%end lflib_trackmap
...
tmpTrackCData = flipud(double(block.DialogPrm(1).Data));

trackXSamples = size(tmpTrackCData, 2);
trackYSamples = size(tmpTrackCData, 1);

trackCData=zeros([trackYSamples trackXSamples 3]);
for i = 1:3; trackCData(:,:,i) = tmpTrackCData(:,:,i) / max(max(tmpTrackCData(:,:,i))); end

trackXSize = trackXSamples / trackSamplesPerM;
trackYSize = trackYSamples / trackSamplesPerM;

[vehCData, vehCMap, vehAData] = imread('lflib_track_vehicle.png');
vehSamplesPerM = 1000;
...
if ~isempty(userData) && ishandle(userData(1)) && ishandle(userData(2)) && ishandle(userData(3))
  hFig = handle(userData(1));
  hTr = handle(userData(2));
  
  hLine = handle(userData(3));
...
block.Dwork(1).Data = double(hFig);
block.Dwork(2).Data = double(hTr);
block.Dwork(3).Data = double(hLine);
...
% Note that inputs are in mm, while the rest is in metres. This is to align
% better with the robot movement model which is in mm.
x = block.InputPort(2).Data / 1000;
y = block.InputPort(3).Data / 1000;
