...
%
% This function is used in conjunction with a 'make' based build process.
function mc_buildLibrariesMake
...
if ~ReadOnlyInstall
    ...
    libraries = setdiff(fieldnames(allLibraries), 'genBC');

    % loop through all libraries
    for ii = 1:length(libraries)

        % currently working on library...
        currLib = libraries{ii};

        % build library (if required)
...
elseif any(mcBuildPars.mcBaseDir == ' ')
    ...
    if ~exist(libGenPath, 'dir')
        ...
        try
            ...
            % display error message (command window) and exit
    ...
    % build diretory: <mcBaseDir>\rtwlib\rtwlibbuild
    libGenPath = fullfile(mcBuildPars.mcBaseDir, 'rtwlib', 'rtwlibbuild');
...
if rtwlibNeedsRebuild
    ...
    switch libraryBaseName
        ...
        case 'rtwlib'
            ...
            % RTW main lib
            libObj.sourcePath = fullfile(matlabroot, 'rtw', 'c', 'libsrc');
    ...
    mc_cwAutomation(libProjectPathAndXML, 'build', [libNameNoExt '.mcp']);
    pause(2);
