...
for ii = 1:length(MC9S12DriverDataBlock)
    ...
    if(~isempty(findstr(MC9S12DriverDataBlock{ii}, 'rfComms_Server')))

        % get current settings
        ll = get_param(MC9S12DriverDataBlock{ii}, 'RTWData');
...
clientmask = 0;
for ii = 1:myNumClients
    clientmask = clientmask + clientmasks(myClients(ii));
end
...
for ii = 1:length(MC9S12DriverDataBlock)
    ...
    if(~isempty(findstr(MC9S12DriverDataBlock{ii}, 'rfComms_Server')))
        ...
        set_param(MC9S12DriverDataBlock{ii}, 'RTWData', ll);
