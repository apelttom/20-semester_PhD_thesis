...
if strcmp(config.ProcessCase,'SingleRun')
    ...
    subplot(6,1,2)
    plot(results{iBin,iRandSeed}.Power.time(:), results{iBin,iRandSeed}.Power.signals.values(:)*1e-6,'LineWidth',2)
    xlabel('time [s]')
    ylabel('Power [MW]')
    xlim([30,630])
    
    subplot(6,1,3)
    plot(results{iBin,iRandSeed}.Omega.time(:), results{iBin,iRandSeed}.Omega.signals.values(:),'LineWidth',2)
    xlabel('time [s]')
    ylabel('\Omega [rpm]')
    xlim([30,630])
    
    subplot(6,1,4)
    plot(results{iBin,iRandSeed}.Theta.time(:), results{iBin,iRandSeed}.Theta.signals.values(:),'LineWidth',2)
    ylabel('\theta [°]')
    xlabel('time [s]')
    xlim([30,630])
    
    subplot(6,1,5)
    plot(results{iBin,iRandSeed}.Mg.time(:), results{iBin,iRandSeed}.Mg.signals.values(:),'LineWidth',2)
    ylabel('M_g [Nm]')
    xlabel('time [s]')
    xlim([30,630])
    
    subplot(6,1,6)
    plot(results{iBin,iRandSeed}.TwrDynLoads.time(:), results{iBin,iRandSeed}.TwrDynLoads.signals.values(:),'LineWidth',2)
    ylabel('M_{yT} [Nm]')
    xlabel('time [s]')
    xlim([30,630])
end
