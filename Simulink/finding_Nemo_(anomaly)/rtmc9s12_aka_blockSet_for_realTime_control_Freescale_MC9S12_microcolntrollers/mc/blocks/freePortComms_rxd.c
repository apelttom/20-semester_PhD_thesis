...
static myUsrBuf *AllocateUserBuffer(uint_T channel, uint_T bufsize, uint8_T data_type_len) {
    ...
    /* initialise the header_received flag */
...
unsigned char    temp;
    ...
    #ifdef VERBOSE
    {
        mexPrintf("freePortComms_rxd: ReverseOrder4 %d:%d:%d:%d >> ", buf[0], buf[1], buf[2], buf[3]);
    }
    #endif /* VERBOSE */
...
unsigned char    temp;
    ...
    #ifdef VERBOSE
    {
        mexPrintf("freePortComms_rxd: ReverseOrder2 %d:%d >> ", buf[0], buf[1]);
    }
    #endif /* VERBOSE */
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    ...
    if(PORT > 2) {
        ...
        int_T     *IWork       = ssGetIWork(S);                    /* IWork[0] : channel number */
        myUsrBuf  *admin       = freecomTelBuf[IWork[0]];       /* admin structure of this channel */
        uint_T       size           = admin->buf_size;
        uint8_T   *buf           = admin->buf;
        
        myCOMPort *adminP      = ssGetPWorkValue(S, 0);            /* adminP->hCom */
        ...
        if(FORMAT == 0) {
            ...
            /* check COM port for newly arrived data (unless a header has already been received) */
            if((admin->header_received == 0) && (checkCOMforData(adminP->hCom) >= 4)) {
                ...
                /* read the HEADER, now ready to receive data */
                admin->header_received = 1;

            } /* at least 4 bytes are available (HEADER) */

        } else {

            if(checkCOMforData(adminP->hCom) > 0) {

                /* a data byte is available -> ready to receive data */
                admin->header_received = 1;        /* using admin[0] in this case... */

            }

        }

        /* at this stage, the HEADER should have been received (if any...) */
        if(admin->header_received) {

            /* only attempt to read data from the serial port once the entire remainder of the packet has arrived... */
            if(checkCOMforData(adminP->hCom) >= size) {
                ...
                /* host DLL -> echo... */
                {
                    ...
                    mexPrintf("RxD < Channel[%d] ", IWork[0]);
                ...
                /* reset variable 'header_rceived' for the next packet */
                admin->header_received = 0;

            } /* entire data packet is in the buffer... */

        } /* header_received = true*/

        /* packet fully received -> descramble and copy to output */            
