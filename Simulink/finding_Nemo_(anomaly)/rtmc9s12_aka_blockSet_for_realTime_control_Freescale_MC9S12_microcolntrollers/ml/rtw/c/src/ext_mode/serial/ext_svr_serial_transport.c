 ...
 *
 * File: ext_svr_transport.c     $Revision: 1.1.6.6 $
 *
 * Abstract:
...
boolean_T error = EXT_NO_ERROR;
    ...
    // local function name... debugging only
    DEFINE_DEBUG_FNAME("ExtGetHostPkt")

    PRINT_DEBUG_MSG_LVL3("IN");
    PRINT_DEBUG_MSG_NL3;

    /*
     * EXT_BLOCKING should be set to '0' to enter the following blocking section.
     * This is only suitable on a real-time target, where this section can be 
     * interrupted / preempted by the OS to run the actual RT model code.
     *
     * On rtmc9s12-target EXT_BLOCKING is set to '0', i. e. the following section
     * is entered. Also see the explanation at the beginning of this file.
     *
     * fw-06-10
     */
    #if EXT_BLOCKING == 0
    ...
    /* at this stage the requested number of bytes should be available in the RX ring buffer */
...
boolean_T error = EXT_NO_ERROR;
    ...
    // local function name... debugging only
    DEFINE_DEBUG_FNAME("ExtSetHostPkt")
...
void ExtModeSleep(
    const ExtUserData   *UD,
    const long_T         sec,       /* # of secs to wait        */
    const long_T         usec)      /* # of micros secs to wait */
{
