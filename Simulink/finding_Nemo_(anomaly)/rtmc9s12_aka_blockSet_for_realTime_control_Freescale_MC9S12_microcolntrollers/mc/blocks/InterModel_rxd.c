/******************************************************************************/
/*                                                                            */
/* Name: InterModel_rxd.c  - S-Function for the communication between models  */
/*                                                                            */
/******************************************************************************/
...
#define S_FUNCTION_NAME InterModel_rxd
...

	{
		...
		if((FreeBufferAdminVar = mexGetVariable("base", "FreeBufferAdminVar")) == NULL) {
			...
			if((admin = AllocateUserBuffer(CHANNEL_NO, buf_size, data_type)) == NULL) {
				...
				mexErrMsgTxt("FreePortComms_rxd: Problem during memory allocation [insufficient memory].\n");
	...
	/* host S-Function (DLL) */
	{
		...
		if(admin->access_count == 1) {
			...
			if((FreeBufferAdminVar = mexGetVariable("base", "FreeBufferAdminVar")) == NULL) {
				...
				mexErrMsgTxt("s0_usrtel_rxd: Error accessing workspace variable FreeBufferAdminVar (mdlTerminate).\n");
...
// this is only required if the format of the call-up parameters is modified... (FW-06-01)
   ...
   #include "simulink.c"
#else
# error "Attempt to use InterModel_rxd.c as non-inlined S-function"
#endif
