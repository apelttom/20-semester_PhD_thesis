...
void rtwCAPI_CountSysRan(const rtwCAPI_ModelMappingInfo *mmi,
			 int                            *count)
{
    sysRanDType **sysRan = rtwCAPI_GetSystemRan(mmi);
    int numSys    = rtwCAPI_GetNumSystems(mmi);
    int nCMMI     = rtwCAPI_GetChildMMIArrayLen(mmi);
    int numSysRan = 0;
    int i;
...
{
    int       numSys      = rtwCAPI_GetNumSystems(mmi);
    sysRanDType **mmiSysRan = rtwCAPI_GetSystemRan(mmi);
    int       nCMMI       = rtwCAPI_GetChildMMIArrayLen(mmi);
    int       *mmiSysTid  = rtwCAPI_GetSystemTid(mmi);
    int       idx         = *fillIdx;
    const int *mmiConSys  = rtwCAPI_GetContextSystems(mmi);
    int i;
