...
mySendBlocks = get_param(find_system(gcs, 'LookUnderMasks', 'all', 'Name', 'SEND_USR_DATA'), 'Parent');     % get all TxD blocks
...
if(length(unique(kk)) < length(kk))
    ...
    disp('s0_userData_txd: Warning... at least 2 TxD blocks have been allocated the same channel number!')
end
