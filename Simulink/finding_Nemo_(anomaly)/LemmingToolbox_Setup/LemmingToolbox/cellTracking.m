...
while true
    ...
    newCellTable(nextPosInCellTable, :) = [newX(newMatch), newY(newMatch), cellTable(oldMatch, 3)];
...
if numLost > 0 && numNewFound > 0
    ...
    while true
        ...
        % Save new found cell in output
        newCellTable(nextPosInCellTable, :) = [newX(newMatch), newY(newMatch), lostCellTable(oldMatch, 3)];
        nextPosInCellTable = nextPosInCellTable + 1;
    end
...
for notFoundIdx = newFoundIdx
    newCellTable(nextPosInCellTable, :) = [newX(notFoundIdx), newY(notFoundIdx), nextCellID];
...
for notFoundIdx = find(~foundOld)
    lostCellTable(end+1, :) = [oldX(notFoundIdx), oldY(notFoundIdx), cellTable(notFoundIdx, 3), 0]; %#ok<AGROW>
end

%% Increase "not-found time" for lost cells and delete cells which are lost
%% already too long
lostCellTable(:, 4) = lostCellTable(:, 4) + 1;
lostCellTable(lostCellTable(:, 4) > lostCellSurvivalTime, :) = [];
