 ...
 *
 * File: ext_svr_transport.h     $Revision: 1.1.6.1 $
 *
 * Abstract:
...
extern boolean_T     ExtOpenConnection(ExtUserData *UD,
...
extern const char_T *ExtProcessArgs(ExtUserData   *UD,
                                    const int_T    argc,
                                    const char_T  *argv[]);
...
extern boolean_T     ExtGetHostPkt(const ExtUserData *UD,
                                   const int_T        nBytesToGet,
                                   int_T             *nBytesGot,
                                   char_T            *dst);

extern boolean_T     ExtSetHostPkt(const ExtUserData *UD,
                                   const int_T        nBytesToSet,
                                   const char_T      *src,
                                   int_T             *nBytesSet);

extern void          ExtModeSleep(const ExtUserData *UD,
                                  const long_T       sec,  
                                  const long_T       usec);

extern void          ExtForceDisconnect(ExtUserData *UD);

/* called from ext_svr/ExtSetWaitForStartPkt()  ... necessary because ext_svr functions can pass pointers to 
   structure extUserData (structure defined in ext_svr_transport.c, pointer defined in ext_svr.c) but they 
   cannot dereference the structure entries...  fw-06-07 */
extern void          ExtSetWaitForStartPktFromHost(ExtUserData *UD, boolean_T state);
