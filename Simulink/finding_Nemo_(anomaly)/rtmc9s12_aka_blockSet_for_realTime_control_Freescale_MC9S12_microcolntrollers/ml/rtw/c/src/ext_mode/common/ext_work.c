 ...
 *
 * File: ext_work.c     $Revision: 1.1.6.7 $
 *
 * Abstract:
 *   
 * 
...
/* the reset values of the following variables get restored in rt_ExtModeShutdown (ext_svr.c) */
int_T           startModel  = FALSE;
TargetSimStatus modelStatus = TARGET_STATUS_WAITING_TO_START;
                             ...
                             SEM_ID         startStopSem)
{
    ...
    commSem   = semBCreate(SEM_Q_PRIORITY, SEM_EMPTY);
    semGive(commSem);
...
void rtExtModePauseIfNeeded(RTWExtModeInfo *ei,
    ...
    DEFINE_DEBUG_FNAME("rtExtModePauseIfNeeded")

   while((modelStatus==TARGET_STATUS_PAUSED) && !startModel && !(*stopReqPtr)) {

        /* not used (empty function call)  -  fw-06-07 */
    ...
    /* reset to FALSE - if we were stepped we want to
...
void rtExtModeWaitForStartPkt(RTWExtModeInfo *ei,
    ...
    // local function name... debugging only
    DEFINE_DEBUG_FNAME("rtExtModeWaitForStartPkt")
    ...
    if (modelStatus != TARGET_STATUS_PAUSED) {
        ...
        modelStatus = TARGET_STATUS_RUNNING;
        //PRINT_DEBUG_MSG_LVL1("modelStatus = TARGET_STATUS_RUNNING");
        //PRINT_DEBUG_MSG_NL1;
