...
//static uint8_T  tempSendPktBuf[1024];   /* used in conjunction with targets that need byte re - ordering -- fw - 02 - 05 */
    ...
    long_T             timeOutUSecs)
{
    ...
    DEFINE_DEBUG_FNAME("ExtTargetPktPending")
    ...
    while (1) {
        ...
        /* modified callup syntax: 'MIN_PACKET_SIZE' byte pending  --  fw-07-07 */
        error = ExtPktPending(userData->portDev, pending, MIN_PACKET_SIZE);
        ...
        if (*pending || !timeLimit) {

            #if DEBUG_MSG_LVL > 0
            if (*pending) {

                PRINT_DEBUG_MSG_LVL5("Detected pending pkt has been acknowledged and stored in FIFO (from where it can be retrieved)\n");
        ...
        if (difftime(currTime, startTime) > timeLimit) {
            ...
            PRINT_DEBUG_MSG_LVL5("Timeout occurred while searching for pending pkt\n");
...
UserData    *userData = (UserData *)esGetUserData(ES);
    ...
    // local function name... debugging only
    DEFINE_DEBUG_FNAME("ExtSetTargetPkt")
    ...
    char_T            *dst)       /* out */
{
    ...
    // local function name... debugging only
    DEFINE_DEBUG_FNAME("ExtGetTargetPkt")

    PRINT_DEBUG_MSG_LVL3("Transparent call to ExtGetPkt\n");
...
PUBLIC void ExtCloseConnection(ExternalSim *ES) {
    ...
    // local function name... debugging only
    DEFINE_DEBUG_FNAME("ExtCloseConnection")
...
PUBLIC void ExtOpenConnection(ExternalSim *ES) {
    ...
    // local function name... debugging only
    DEFINE_DEBUG_FNAME("ExtOpenConnection")
...
UserData    *ud = (UserData *)calloc(1, sizeof(UserData));
    ...
    // local function name... debugging only
    DEFINE_DEBUG_FNAME("ExtUserDataCreate")
...
PUBLIC void ExtUserDataDestroy(UserData *userData) {
    ...
    // local function name... debugging only
    DEFINE_DEBUG_FNAME("ExtUserDataDestroy")
